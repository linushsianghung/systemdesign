...
```postgresql
-- GENERATED ALWAYS AS IDENTITY: PostgreSQL will always generate a value for the column. An error will occur if you attempt to manually INSERT a value into this column.
-- GENERATED BY DEFAULT AS IDENTITY: PostgreSQL will generate a value by default, but you can explicitly provide a value during an INSERT if needed (e.g., when importing data). 
...
-- Index for user lookup by email (for login)
CREATE INDEX idx_users_email ON users(email);

-- businesses Table
CREATE TABLE businesses (
    business_id INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
-- Geospatial index for location-based queries (PostgreSQL has natie support)
CREATE INDEX idx_business_location ON businesses USING GIST (
    ll_to_earth(latitude, longitude)                                                            
);

-- reviews Table with Table Partitioning (PostgreSQL Declarative Partitioning)
CREATE TABLE reviews (
    ...
    PRIMARY KEY (review_id, created_at) -- Must include partition in PK
) PARTITION BY RANGE (created_at);

-- Create partitions for each year
CREATE TABLE reviews_2026 PARTITION OF reviews FOR VALUES FROM ('2026-01-01') TO ('2027-01-01');
CREATE TABLE reviews_2027 PARTITION OF reviews FOR VALUES FROM ('2027-01-01') TO ('2028-01-01');
CREATE TABLE reviews_2028 PARTITION OF reviews FOR VALUES FROM ('2028-01-01') TO ('2029-01-01');

-- Index 1 (Most important): Get reviews for a business (sorted by date)
CREATE INDEX idx_reviews_business_create ON reviews(business_id, created_at DESC);

-- Index2: Get reviews by a user (sorted by date)
CREATE INDEX idx_reviews_user_create ON reviews(user_id, created_at DESC);

-- Index3: For analytics/moderation to find recent reviews
CREATE INDEX idx_reviews_create ON reviews(created_at DESC);

-- Index4: For soft delete filtering (partial index)
CREATE INDEX idx_reviews_active ON reviews(business_id, created_at DESC) WHERE deleted_at IS NULL;

-- Trigger for auto-update updated_at timestamp (PostgreSQL doesn't have ON UPDATE like MySQL)
