...
- _Wait Event Analysis_: Sometimes plan is fine but query still slow...
  ...
  |:-----|:----------------|:-------------|:------------------------------------------------------------------------------------|
  | 1204 | NULL            | NULL         | Running: This process is actively using the CPU                                     |
...
- [Blocked] _Lock Analysis_: Sometimes plan is fine but query still slow...
  ```sql
  -- Method 1: A function (available since PostgreSQL 9.6) that identifies sessions that are waiting for a lock and list of the PIDs of the sessions holding those locks.
  SELECT pid, username, query AS blocked_query, pg_blocking_pids(pid) AS blocking_pid FROM pg_stat_activity WHERE cardinality(pg_blocking_pids(pid)) > 0;
